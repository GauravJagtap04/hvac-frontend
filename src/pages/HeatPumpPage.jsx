          ...
          <Card className="bg-background">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <Label htmlFor="cop-rated">Rated COP: </Label>
                    <span>{hvacParameters.copRated}</span>
                  </div>
                  <Slider
                    id="cop-rated"
                    min={1.0}
                    max={5.0}
                    step={0.1}
                    value={[Number(hvacParameters.copRated)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("copRated")(null, values[0]);
                      }
                    }}
                    className="relative h-4"
                    disabled={
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <Label htmlFor="cop-min">Minimum COP: </Label>
                    <span>{hvacParameters.copMin}</span>
                  </div>
                  <Slider
                    id="cop-min"
                    min={1.0}
                    max={3.0}
                    step={0.1}
                    value={[Number(hvacParameters.copMin)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("copMin")(null, values[0]);
                      }
                    }}
                    className="relative h-4"
                    disabled={
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    value={
                      roomParameters.mode === "heating"
                        ? [Number(hvacParameters.supplyTempHeating)]
                        : [Number(hvacParameters.supplyTempCooling)]
                    }
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        roomParameters.mode === "heating"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <Label htmlFor="defrost-temp-threshold">
                      Defrost Temperature Threshold:{" "}
                    </Label>
                    <span>{hvacParameters.defrostTempThreshold}°C</span>
                  </div>
                  <Slider
                    id="defrost-temp-threshold"
                    min={0}
                    max={10}
                    step={0.5}
                    value={[Number(hvacParameters.defrostTempThreshold)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("defrostTempThreshold")(
                          null,
                          values[0]
                        );
                      }
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleHVACParameterChange("refrigerantType")(null, value)
                    }
                    disabled={
                      (isSimulationRunning && !isSimulationPaused) ||
                      hasInvalidFields
