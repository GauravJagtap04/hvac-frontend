...
const SimulationPage = () => {
  ...
  const [invalidFields, setInvalidFields] = useState({
    ...
    breadth: false,
    height: false,
    ...Object.keys(safeObject(hvacParameters.zones)).reduce((acc, zone) => {
      acc[`zone_${zone}`] = false;
      return acc;
    }, {}),
  });
      ...
      <div className="py-6 px-3 sm:px-6 md:px-6 lg-px-8 bg-background shadow-md w-full mx-auto">
        <div className="flex items-center justify-between w-full">
          <h1 className="text-3xl font-[900] mb-6 font-rubik text-primary">
            Variable Refrigerant Flow System Simulator
          </h1>
          <div>
            {/* Connection status */}
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span>{hvacParameters.maxCapacityKw}kW</span>
                  </div>
                  <Slider
                    id="max-capacity"
                    min={1}
                    max={10}
                    step={0.5}
                    value={[Number(hvacParameters.maxCapacityKw)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("maxCapacityKw")(
                          null,
                          values[0]
                        );
                      }
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span>{hvacParameters.minCapacityKw}kW</span>
                  </div>
                  <Slider
                    id="min-capacity"
                    min={0.5}
                    max={5}
                    step={0.5}
                    value={[Number(hvacParameters.minCapacityKw)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("minCapacityKw")(
                          null,
                          values[0]
                        );
                      }
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    max={5.0}
                    step={0.1}
                    value={[Number(hvacParameters.cop)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("cop")(null, values[0]);
                      }
                    }}
                    className="relative h-4"
                    disabled={
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <Label htmlFor="supply-temp">Supply Temperature: </Label>
                    <span>{hvacParameters.supplyTemp}°C</span>
                  </div>
                  <Slider
                    id="supply-temp"
                    min={5}
                    max={30}
                    step={0.5}
                    value={[Number(hvacParameters.supplyTemp)]}
                    onValueChange={(values) => {
                      if (values && values.length > 0) {
                        handleHVACParameterChange("supplyTemp")(
                          null,
                          values[0]
                        );
                      }
                  ...
                  <div className="mt-4 flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        handleHVACParameterChange("heatRecovery")(null, checked)
                      }
                      disabled={
                        (isSimulationRunning && !isSimulationPaused) ||
                        hasInvalidFields
                    ...
                    />
                    <Label htmlFor="heat-recovery">Heat Recovery</Label>
                  </div>
                </div>
              </div>
            </CardContent>
